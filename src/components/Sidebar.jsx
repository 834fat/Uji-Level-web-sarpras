...
import { FaHome, FaBoxOpen, FaDoorClosed, FaCartArrowDown, FaUser, FaFolder, FaCog, FaKey } from "react-icons/fa"; // Import the required icons
import { FaGear } from "react-icons/fa6"; // Import the "FaGear" icon

const Sidebar = () => {
  const [open, setOpen] = useState(true);
  const [userRole, setUserRole] = useState("");
  const [selectedMenuIndex, setSelectedMenuIndex] = useState(-1); // State to store the selected menu index
  ...
  const Menus = [
    ...
    { title: "Role Permissions", gap: true, path: "/RolePermissions" },
  ...
  const renderMenuItem = (Menu) => {
    ...
    } else if (Menu.title === "Role Permissions") {
      return (
        <React.Fragment>
          <span className={`ml-2 ${!open && "opacity-0"}`} style={{ fontFamily: 'Livvic, sans-serif' }}>
            {Menu.title}
      ...
      return (
        <React.Fragment>
          <img
            ...
            alt={Menu.title}
            className={`w-5 h-5 ${!open && "opacity-0"}`}
          />
          <span className={`ml-2 ${!open && "opacity-0"}`} style={{ fontFamily: 'Livvic, sans-serif' }}>
            {Menu.title}
          </span>
        ...
        <ul className={`pt-8 ${!open && "hidden"}`}> {/* Adjust margin-top here */}
          ...
          {Menus.map((Menu, index) => (
            <li
              ...
              ${Menu.gap ? "mt-3" : "mt-2"} ${index === selectedMenuIndex && "bg-gray-700"}`} // Adjust margin-top for gaps
